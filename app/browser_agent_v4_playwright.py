...
GOAL = "From the Playwright homepage, open the Python Installation documentation."
...
def observe(page):
    ...
    for i in range(
        ...
        if normalized_text in seen_link_texts:
            continue
...
- If the goal does not require search, do not use site search unnecessarily.
- On a product homepage, a language/framework link may be required before opening its specific documentation.
- "Get started" can be relevant when the user requests installation/setup documentation.
...
def target_reached(page):
    ...
    return (
        "installation" in title
        and
        "playwright python" in title
        and
        "/python/docs/intro" in url
    )
...
def main():
    print()
    print("=" * 60)
    print("REAL PLAYWRIGHT DOCS JEV AGENT")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        if success:
            print()
            print(
                "REAL_PLAYWRIGHT_AGENT=PASS"
            )
        else:
            print()
            print(
                "REAL_PLAYWRIGHT_AGENT=FAIL"
            )
