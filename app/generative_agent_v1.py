...
Replace A with the chosen action letter.
...
def parse_generation(
    ...
    # Secondary parser:
    # lets us observe whether a normal
    # agent framework could salvage
    # JSON surrounded by prose.
    if choice is None:

        match = re.search(
            r'\{\s*"choice"\s*:\s*"([A-Z])"\s*\}',
            raw,
        )
...
def main():
    print()
    print("=" * 60)
    print("GENERATIVE GEMMA BROWSER BASELINE")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        if success:
            print(
                "GENERATIVE_BASELINE=PASS"
            )
        else:
            print(
                "GENERATIVE_BASELINE=FAIL"
            )
