...
def descendant_option_names(
    combobox_node,
    all_nodes,
):
    """
    Find AX option descendants belonging to a combobox.

    Browser accessibility trees generally expose
    childIds / nodeId relationships even when the
    actual dropdown menu is collapsed and options
    have visibility=0.
    """
...
def all_option_names(
    ...
    """
    Fallback for browser AX trees where collapsed
    <select> options are present but not connected
    through childIds in the flattened snapshot.
    """
...
def build_candidates(
    ...
    for node in all_nodes:
        ...
        # ---------------------------------
        # SELECT / COMBOBOX ACTIONS
        # ---------------------------------

        if role == "combobox":

            option_names = (
                descendant_option_names(
                    node,
                    all_nodes,
                )
            )

            # Some collapsed native <select> controls
            # expose their options separately in the
            # AX snapshot. If there is only one
            # combobox, association is unambiguous.
            if (
                not option_names
                and
                len(
            ...
            element_name = (
                name
                if name
                else "unnamed combobox"
            )

            for option_name in option_names:

                # Do not propose selecting the value
...
- If the goal asks to select or choose an option from a list, dropdown, or combobox, select the exact requested option before submitting.
- If the requested option is already the current combobox value and a submit/confirm control is available, submit or confirm it.
- Never submit a form before the requested textbox or selection value has been correctly set.
...
- Return the best action label."""
    ...
    #
    # If there is only one legal browser action,
    # there is nothing for a comparative scorer to
    # decide. Execute it deterministically instead
    # of calling OpenJEV.
    # -------------------------------------------------
...
def main():
    ...
    print("=" * 70)
    print(
        "GENERIC OPENJEV + BROWSERGYM + MINIWOB V2"
    )
    print("=" * 70)
